...
class FTManager(Configurable):
    ...
    def __init__(
        ...
        # IMPORTANT:
        # Do not pass rank0_synchronization_only, checkpoint_transport,
        # copy_pseudogradients_to_cpu, etc. here.
        # The installed TorchFT version on Nova does not support those arguments.
    ...
    def rank0_synchronization_only(self) -> bool:
        """
        Compatibility property expected by FTCheckpointManager.

        Nova's FTCheckpointManager reads ft_manager.rank0_synchronization_only,
        but this installed TorchFT version does not support passing
        rank0_synchronization_only into torchft.Manager(...).

        For our current DiLoCo / HeLoCo runs, rank-0-only synchronization is disabled.
        """
        return False
...
def maybe_semi_sync_training(
    ...
    if extend_ft_config.enable and semi_sync_method is not None:
        ...
        if semi_sync_method.lower() == "diloco":
            ...
            # Added for using custom outer optimizer: nesterov / mla / heloco
            from torchtitan.experiments.ft.diloco.outer_optimizer import (
            ...
            # Create the outer optimizer based on the model fragment parameters.
            outer_optimizers = []

            for model_part in model_parts:
                params = [p for p in model_part.parameters() if p.requires_grad]

                #########*********** Commented out to use custom outer optimizer *******############
                # Original TorchTitan DiLoCo outer optimizer:
                #
                # outer_optimizer = torch.optim.SGD(
                #     params, lr=0.7, momentum=0.9, nesterov=True
                # )
                #
                # Now this is handled by build_outer_optimizer(...), so we can select:
                #   outer_optimizer="nesterov"  -> original TorchTitan behavior
                #   outer_optimizer="mla"       -> MLA baseline
                #   outer_optimizer="heloco"    -> HeLoCo outer optimizer

                outer_optimizer = build_outer_optimizer(params, extend_ft_config)
